/**
 * SpaceCheck XR journey PDF — layout ported from Pois Journey `PoisJourneyUI._generatePDFReport`.
 */
...
/**
 * Build and download a SpaceCheck XR journey PDF (Pois Journey layout).
 *
 * @param {{
...
export async function generateJourneyPdfReport(params) {
  const {
    ...
    userEmail = 'Guest',
    extraIssues = [],
    fileName = 'SpaceCheck_XR_Report.pdf',
    onProgress,
  } = params;
  ...
  doc.setFontSize(20);
  doc.setFont('helvetica', 'bold');
  doc.text('SpaceCheck XR Report', margin, 16);
  doc.setFontSize(11);
  doc.setFont('helvetica', 'normal');
  ...
  doc.setFontSize(8);
  doc.text('Generated automatically by SpaceCheck XR', pageW / 2, pdfCtx.y, { align: 'center' });
